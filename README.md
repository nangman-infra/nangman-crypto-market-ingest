...
운영 기준은 이 앱 repo 안의 Rust 코드, schema/ecs 예제, README/runbook, 그리고
실제 ECS/S3/CloudWatch 증거다. 상위 workspace의 legacy 자료나 local harness
예제는 이 앱의 배포 source of truth가 아니다.
...
- market-ingest-app은 S3를 canonical durable store로 유지하면서, 설정된 경우
  NATS `market_live_tick.created.<venue>.<symbol>` subject에 빠른 paper-watch용
  live mark event를 함께 publish한다.
- downstream durability handoff는 여전히 S3 `l1_index` success pointer다. NATS
  `MARKET_LIVE` stream은 모의투자/관측 latency를 줄이는 hot path이며 주문 판단을 하지 않는다.
...
```text
 ...
 market-ingest-app
        └─→ NATS MARKET_LIVE
            subject=market_live_tick.created.<venue>.<symbol>
            schema=market_live_tick_v1
```
...
- `--live-nats-url` 또는 `MARKET_LIVE_NATS_URL`이 설정되면 realtime L0 event에서
  mark price를 만들 수 있는 trade/ticker/book/depth payload를 `market_live_tick_v1`로
  publish한다.
- 기본 subject prefix는 `market_live_tick.created`, 기본 stream은 `MARKET_LIVE`.
  `--live-nats-required`를 켜면 NATS publish 실패가 worker 실패로 이어진다.
  기본값은 false라 S3 L0/L1 적재가 우선이다.
...
`recent_bootstrap_rollups.incomplete_rollups`로 확인한다. 누락이 과거 window
부족 때문이면 `recent_bootstrap_rollups.bootstrap_approval_projection`에서
추가로 필요한 complete day와 예상 full-bootstrap event date를 확인한다.
...
`runner_build_profile`을 포함한다. ECS에 올리는 Docker image는 Rust compile 전에
`NANGMAN_GIT_SHA`와 `NANGMAN_GIT_DIRTY` build arg를 주입해야 한다. git 상태를
...
이 gate는 shell/Python script validation, self-test, repository contract,
Rust fmt/clippy/test, linux/arm64 Docker build, image smoke를 한 번에 실행한다.
AWS/ECR/ECS/S3에는 쓰지 않는다. 배포 승인 전에 로컬 release candidate가 기본
DoD를 만족하는지 확인하기 위한 gate이며, runtime freshness 증명을 대체하지 않는다.
...
- ECS task definition은 FARGATE_SPOT, LINUX/ARM64, readonly root filesystem을 사용.
- ECS service는 desired/running/pending expected 상태와 rollout COMPLETED를 유지.
...
- S3 runtime은 AWS S3 + IAM만 사용한다. S3-compatible endpoint 설정은 앱
  계약에 포함하지 않는다.
