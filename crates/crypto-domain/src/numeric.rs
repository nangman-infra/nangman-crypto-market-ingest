...
///
/// `cmp_scaled` returns a `Result` because aligning can overflow `i128` for
/// extreme inputs; for ordering purposes we fall back to the saturating
...
fn saturating_align(value: i128, scale_delta: u32) -> i128 {
    let mut acc = value;
